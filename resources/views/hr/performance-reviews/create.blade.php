...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 bg-white border-b border-gray-200">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold">Create Performance Review</h1>
        ...
        <form action="{{ route('hr.performance-reviews.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="user_id" id="user_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('user_id', request('employee_id')) == $employee->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="review_date" id="review_date" value="{{ old('review_date', date('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @error('review_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="mt-1 flex items-center space-x-1">
                        @for ($i = 1; $i <= 5; $i++)
                            <div>
                                <input type="radio" name="rating" id="rating_{{ $i }}" value="{{ $i }}" {{ old('rating') == $i ? 'checked' : '' }} class="hidden peer" required>
                                <label for="rating_{{ $i }}" class="cursor-pointer block p-2 border border-gray-300 rounded-full peer-checked:bg-yellow-400 peer-checked:border-yellow-500 hover:bg-yellow-100 transition-colors">
                                    <svg class="h-6 w-6 text-gray-400 peer-checked:text-gray-700" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="achievements" id="achievements" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('achievements') }}</textarea>
                    @error('achievements')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="areas_for_improvement" id="areas_for_improvement" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('areas_for_improvement') }}</textarea>
